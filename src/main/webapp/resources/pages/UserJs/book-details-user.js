$(document).ready(function() {
	 ...
	 if (localStorage.getItem('cid') !== null){
        getBooksByCategory(localStorage.getItem('cid'));
    }
...
function getBooksByCategory(categoryId) {
    $.ajax({
        url: contextPath + "/book?method=display_books_categorywise",
        type: 'POST',
        dataType: 'json',
...
function createWishlistItem(item) {
        ...
        '<div class="col-lg-4 col-md-6 col-sm-6">' +
            '<div class="product__item sale">' +
                '<div class="product__item__pic set-bg" data-setbg="' + contextPath + '/resources/images/books/' + item.bookimage + '">' +
                    '<ul class="product__hover">' +
                        '<li><a href="#" onclick="addToWishlistCheck(' + item.bookId + ')"><img src="' + contextPath + '/resources/images/user/icon/heart.png" alt=""></a></li>' +
                        '<li><a href="#" onclick="viewBook(' + item.bookId + ')"><img src="' + contextPath + '/resources/images/user/icon/view.png" alt=""></a></li>' +
                    '</ul>' +
                '</div>' +
                '<div class="product__item__text">' +
                    '<h6>' + item.bookname + '</h6>' +
    ...
    var $wishlistItem = $(wishlistItemHtml);
    
    // Set the background image after appending the element to the DOM
    $wishlistItem.find('.set-bg').each(function() {
        var bg = $(this).data('setbg');
        if (bg) {
            $(this).css('background-image', 'url(' + bg + ')');
        }
        ...
        success: function(response) {
            if (response.success) {
             ...
             // Update languages
              ...
              /* $("#cartadd").attr("onclick", `addToCartCheck(${bookDetails.id}, ${bookDetails.price})`);
                $("#wishlistadd").attr("onclick", `addToWishlistCheck(${bookDetails.id})`);*/
                 $("#addToCartButton").attr("onclick", `addToCartCheck(${bookDetails.id}, ${bookDetails.price})`);
                $("#addToWishlistButton").attr("onclick", `addToWishlistCheck(${bookDetails.id})`);
            } else {
